...
=begin Nick's demonstrative doodles...
    class Parser < Parslet::Parser
        root(:query)
        rule(:query) { field_or_array.repeat(1,1).as(:term) >> (period >> field_or_array).repeat.as(:term) }
        rule(:field_or_array) { match('\w+') | array }
        rule(:period) { str('.')}
    end
    ...
    class Field
        ...
        def get_value hash
            #raise "not hash! #{hash}" unless hash.kind_of? Hash
            hash[@key]
        end
    end
    ...
    class ArrayOfHashes
        ...
        def [] key
            if key.is_a? Numeric
                ...
                ArrayOfHashes.new(@array.map{|h| h[key]})
            end
        end
        def to_s
            @array.join ', '
    ...
    Parser.new.parse("gnip.foo")


              "gnip.matching_rules.[].value"

              #ArrayOfHashes.new [{value: "blah"},{value: "blah2"}]
              #["blah", "blah2"]
              #to_s
              #blah, blah2
              # =>
...
=end
    ...
    def check_converted
        ...
        Dir.glob("#{@config.data_dir}/*#{@config.job_uuid}*.json") do |file|
            unconverted = unconverted + (File.read(file).scan(/\n/).count - 1 )
        end

        #count the lines in CSV files, minus header.
        Dir.glob("#{@config.data_dir}/*#{@config.job_uuid}*.csv") do |file|
    ...
    def convert_files
        ...
        #if keys_template.include?("object.actor.location.generator.displayName") then
        #    p "STOP!"
        #end

        #TODO: dev remove.
        #keys_template.each { |key|
        #   p "Template key: #{key}"
        #}
        ...
        Dir.glob("#{@config.data_dir}/*#{@config.job_uuid}*.json") do |file|
            ...
            #p file
            csv_filename = "#{@config.data_dir}/#{File.basename(file, ".*")}.csv"
...
if __FILE__ == $0  #This script code is executed when running this file.
    ...
    convert = DM_Converter.new(oConfig,oStatus,nil)
